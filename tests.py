...
class TestHomoglyphs(unittest.TestCase):
    ...
    def test_strategy(self):
        variants = Homoglyphs(["LATIN"], strategy=STRATEGY_IGNORE)._get_char_variants(
            "ё"
        )
        self.assertEqual(variants, ["ё"])

        variants = Homoglyphs(["LATIN"], strategy=STRATEGY_LOAD)._get_char_variants(
            CIRILLIC_HE
        )
        ...
        variants = Homoglyphs(["LATIN"], strategy=STRATEGY_REMOVE)._get_char_variants(
            CIRILLIC_HE
        )
    ...
    def test_to_ascii(self):
        ...
        ss = Homoglyphs(strategy=STRATEGY_LOAD, ascii_range=range(256)).to_ascii(
            CIRILLIC_HE + "23."
        )
